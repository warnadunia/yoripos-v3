...
export default function RiwayatView({ onOpenKitchen, onOpenSidebar }: { onOpenKitchen?: () => void, onOpenSidebar: () => void }) {
    ...
    const allTransactions = [
        { id: 1, date: '12', month: '06', monthIdx: 5, name: 'Agam Warmindo', inv: 'INV - 123456789', amount: '534.234', method: 'CASH', badge: 'bg-emerald-100 text-emerald-700' },
        { id: 2, date: '12', month: '06', monthIdx: 5, name: 'SS Telur', inv: 'INV - 123456790', amount: '230.234', method: 'QRIS', badge: 'bg-indigo-100 text-indigo-700' },
        { id: 3, date: '11', month: '06', monthIdx: 5, name: 'Bakpia 3 Generasi', inv: 'INV - 123456791', amount: '123.098', method: 'QRIS', badge: 'bg-indigo-100 text-indigo-700' },
        { id: 4, date: '10', month: '06', monthIdx: 5, name: 'Lisa Untari', inv: 'INV - 123456792', amount: '34.098', method: 'TRANSFER', badge: 'bg-amber-100 text-amber-700' },
        { id: 5, date: '08', month: '06', monthIdx: 5, name: 'Modang', inv: 'INV - 123456793', amount: '12.098', method: 'TRANSFER', badge: 'bg-amber-100 text-amber-700' },
        { id: 6, date: '15', month: '05', monthIdx: 4, name: 'Toko Sejahtera', inv: 'INV - 987654321', amount: '450.000', method: 'CASH', badge: 'bg-emerald-100 text-emerald-700' },
    ];
        ...
        <div className="w-full pb-28 relative bg-slate-50 min-h-screen">
            ...
            <div className="w-full bg-emerald-500 rounded-b-3xl pb-8 shadow-sm">
                ...
                <div className="mx-4 bg-emerald-50 rounded-2xl p-4 shadow-sm mb-4">
                    <div className="flex justify-between items-center mb-3">
                        <span className="text-xs font-bold text-emerald-800">Metode Pembayaran</span>
                        <div className="flex gap-2">
                            <span className="flex items-center gap-1 text-[9px] text-emerald-700 font-bold"><span className="w-2 h-2 rounded-sm bg-emerald-500"></span>CASH</span>
                            <span className="flex items-center gap-1 text-[9px] text-indigo-700 font-bold"><span className="w-2 h-2 rounded-sm bg-indigo-500"></span>QRIS</span>
                            <span className="flex items-center gap-1 text-[9px] text-amber-600 font-bold"><span className="w-2 h-2 rounded-sm bg-amber-400"></span>TF</span>
                        </div>
                    </div>
                        ...
                        {monthlyData.map((data, i) => {
                            ...
                            return (
                                <div key={i} onClick={() => setSelectedMonth(i)} className={`flex-1 flex flex-col justify-end h-full w-full opacity-80 cursor-pointer transition-all duration-300 ${isActive ? 'opacity-100 scale-110 shadow-sm' : 'hover:opacity-100'}`}>
                                    {/* Stack 1: Transfer */}
                                    <div className={`w-full rounded-t-[2px] transition-all duration-300 ${isActive ? 'bg-amber-400' : 'bg-amber-300'}`} style={{ height: `${data.tfPct}%` }}></div>
                                    {/* Stack 2: QRIS */}
                                    <div className={`w-full transition-all duration-300 ${isActive ? 'bg-indigo-500' : 'bg-indigo-400'}`} style={{ height: `${data.qrisPct}%` }}></div>
                                    {/* Stack 3: Cash */}
                                    <div className={`w-full rounded-b-[2px] transition-all duration-300 ${isActive ? 'bg-emerald-600' : 'bg-emerald-400'}`} style={{ height: `${data.cashPct}%` }}></div>
                                </div>
                            );
                        })}
                    </div>
            ...
            <div className="mx-4 -mt-6 relative z-10">
                <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-200">
                    ...
                    <p className="text-2xl font-black text-slate-800 mb-4">Rp {activeData.total}</p>

                    <p className="text-[9px] text-slate-400 italic mb-2">*Klik kartu untuk filter metode, Double-Klik untuk reset filter.</p>

                    <div className="flex gap-2">
                        {/* KARTU CASH */}
                        <div
                            onClick={() => setActivePaymentFilter(prev => prev === 'CASH' ? null : 'CASH')}
                            className={`flex-1 bg-emerald-50 rounded-xl p-2.5 border cursor-pointer transition-all duration-200 transform-gpu backface-hidden select-none ${activePaymentFilter === 'CASH' ? 'border-emerald-500 ring-2 ring-emerald-200 shadow-md scale-[1.02]' : 'border-emerald-100 hover:border-emerald-300'}`}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className="text-[10px] font-bold text-emerald-700 uppercase">Cash</span>
                                <svg className="w-3.5 h-3.5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                            </div>
                            <p className="text-[10px] text-emerald-600 mb-0.5">Rp</p>
                            <p className="text-xs font-black text-emerald-800 leading-none truncate">{activeData.cash}</p>
                        ...
                        <div
                            ...
                            className={`flex-1 bg-indigo-50 rounded-xl p-2.5 border cursor-pointer transition-all duration-200 transform-gpu backface-hidden select-none ${activePaymentFilter === 'QRIS' ? 'border-indigo-500 ring-2 ring-indigo-200 shadow-md scale-[1.02]' : 'border-indigo-100 hover:border-indigo-300'}`}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className="text-[10px] font-bold text-indigo-700 uppercase">QRIS</span>
                            ...
                            <p className="text-[10px] text-indigo-600 mb-0.5">Rp</p>
                            <p className="text-xs font-black text-indigo-800 leading-none truncate">{activeData.qris}</p>
                            <span className="text-[9px] font-bold text-indigo-400 mt-1.5 block">{activeData.qrisInv} Trx</span>
                        </div>

                        {/* KARTU TRANSFER */}
                        <div
                            onClick={() => setActivePaymentFilter(prev => prev === 'TRANSFER' ? null : 'TRANSFER')}
                            className={`flex-1 bg-amber-50 rounded-xl p-2.5 border cursor-pointer transition-all duration-200 transform-gpu backface-hidden select-none ${activePaymentFilter === 'TRANSFER' ? 'border-amber-500 ring-2 ring-amber-200 shadow-md scale-[1.02]' : 'border-amber-100 hover:border-amber-300'}`}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className="text-[10px] font-bold text-amber-700 uppercase">Trf</span>
                            ...
                            <p className="text-[10px] text-amber-600 mb-0.5">Rp</p>
                            <p className="text-xs font-black text-amber-800 leading-none truncate">{activeData.tf}</p>
            ...
            <div className="mx-4 mt-4 bg-white rounded-3xl p-4 shadow-sm border border-slate-200">
                <div className="flex justify-between items-center mb-4">
                    ...
                    {activePaymentFilter && (
                        <span className={`text-[9px] font-bold px-2 py-1 rounded-md ${activePaymentFilter === 'CASH' ? 'bg-emerald-100 text-emerald-700' : activePaymentFilter === 'QRIS' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'}`}>
                ...
                <div className="flex flex-col space-y-4">
                    {filteredTransactions.length === 0 ? (
                        <div className="text-center py-6 text-slate-400 text-sm font-medium">Tidak ada transaksi untuk filter ini.</div>
                    ) : filteredTransactions.map(trx => {
                        const isExpanded = expandedTrx === trx.id;

                        return (
                            <div key={trx.id} className="border-b border-slate-100 pb-4 last:border-0 last:pb-0 transition-opacity duration-300">
                                ...
                                <div className="flex items-center justify-between cursor-pointer active:opacity-70" onClick={() => setExpandedTrx(isExpanded ? null : trx.id)}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-500 font-mono mt-0.5">{trx.inv}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-2 mb-4 bg-slate-50 p-3 rounded-lg border border-slate-100">
                                            <div className="flex justify-between items-start">
                                                <div className="flex items-start gap-1">
                                                    ...
                                                    <div>
                                                        <p className="text-xs font-bold text-slate-700">Telur AKAM Herbal</p>
                                                        <p className="text-[10px] text-slate-500 mt-0.5">2 x Rp 25.000</p>
                                                    </div>
                                                </div>
                                                <span className="text-xs font-bold text-slate-700">Rp 50.000</span>
                                            </div>
                                            <div className="flex justify-between items-start">
                                                <div className="flex items-start gap-1">
                                                    <span className="text-slate-400 text-xs mt-0.5 font-bold">-</span>
                                                    <div>
                                                        <p className="text-xs font-bold text-slate-700">Beras Premium</p>
                                                        <p className="text-[10px] text-slate-500 mt-0.5">1 x Rp 73.098</p>
                                                    </div>
                                                </div>
                                                <span className="text-xs font-bold text-slate-700">Rp 73.098</span>
                                            </div>
                                        </div>

                                        {/* Simple Action Buttons */}
                                        <div className="flex gap-2">
                                            <button className="flex-1 rounded-lg text-xs font-bold py-2.5 flex items-center justify-center gap-1 border border-slate-300 bg-white text-slate-600 hover:bg-slate-100 transition-colors transform-gpu active:scale-95 shadow-sm">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg> Cetak
                                            </button>
                                            <button className="flex-1 rounded-lg text-xs font-bold py-2.5 flex items-center justify-center gap-1 border border-slate-300 bg-white text-emerald-600 hover:bg-emerald-50 hover:border-emerald-200 transition-colors transform-gpu active:scale-95 shadow-sm">
